...
function EditLecturer({ openModal, closeModal, handleReload, data }) {
    ...
    const { editLecturer } = useManageLecturerApi();
    ...
    useEffect(() => {
        if (data) {
            setFormData({
                ...
                email: data?.email,
                hocHamHocVi: data?.hoc_ham_hoc_vi,
                chTg: data?.ch_tg,
            });
        }
    }, [data]);
    ...
    const handleEditLecturer = async () => {
        ...
        const result = await editLecturer(data.id, formData);
...
export default memo(EditLecturer);
